//! Flexible password generator based on the [`Cryptirust`](../cryptirust/index.html) library.
//!
//! # Installation
//! ```bash
...
pub fn main() {
    ...
    let mut generator = Generator::new_custom(
        match args.style {
            WordList::English() => word_list::eff::list(),
            WordList::Italian() => word_list::italian::list(),
            WordList::CV() => word_list::cv::list(),
        },
        args.depth,
    )
    .unwrap();
